...
void controller_CalcularIva(LinkedList* pArrayListPassenger)
{
	ll_map(pArrayListPassenger,calcularPrecioFinal);
}

LinkedList* controller_FiltrarPorClase(LinkedList* pArrayListPassenger)
{
	 int class;
	 int verif=0;
	 LinkedList* FList;
	 while(verif==0)
	 {
		...
		printf("3 ");
		printType(3);
		verif=Utn_getInt("Ingrese el tipo de Pasajero",0,4,&class);
	 }
	 switch(class)
	 {
	 case 1:
		 FList=ll_filter(pArrayListPassenger,Passenger_filtrarPorClase1);
		 break;
	 case 2:
		 FList=ll_filter(pArrayListPassenger,Passenger_filtrarPorClase2);
		 break;
	 case 3:
		 FList=ll_filter(pArrayListPassenger,Passenger_filtrarPorClase3);
		 break;
	 }
	 return FList;
}

void controler_VerifyFiltredList(LinkedList* pArrayListPassenger,char* path)
{
	LinkedList* filtredList;
	filtredList=ll_newLinkedList();
	if(filtredList!=NULL)
	{
		int auxContain;
		controller_loadFromBinary(path,filtredList);
		auxContain=ll_containsAll(pArrayListPassenger,filtredList);
		if(auxContain)
		{
			printf("Ha ocurrido un problema:\n");
			printf("Uno o mas pasajeros no de la lista filtrada no se encuentran el la lista principal\n");
			system("pause");
		}else{
			printf("Todos los pasajeros de la lista filtrada se encuentran en la lista principal\n");
			system("pause");
		}
	}
}
