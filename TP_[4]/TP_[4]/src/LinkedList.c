...
int ll_remove(LinkedList* this,int index)
{
    ...
    if(this!=NULL)
    {
    	...
    	{
    		delete=getNode(this,index);

    		if(delete!=NULL)
    		{
    			if(index)//if index != 0
				{
    				prevDelete=getNode(this,index-1);
    				if(prevDelete!=NULL)
    				{
    					prevDelete->pNextNode=delete->pNextNode;
    				}
				}
				else //index=0
				{
					this->pFirstNode=delete->pNextNode;
				}
    			free(delete);
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this!=NULL)
    {
        if(from>=0 && to <= ll_len(this) && to>from)
        {
    	...
    	if(order>-1 && order<2)
    	{
    		...
    		for(i=0;i<len-1;i++)
    		{
    			...
    			for(j=i+1;j<len;j++)
    			{
					...
					auxCmp=pFunc(pElementI,pElementJ);
...
LinkedList* ll_filter(LinkedList* this, int(*fn)(void*))
{
	...
	if(this!=NULL && fn!=NULL)
	{
		...
		if(FiltredList!=NULL)
		{
			void* filtredElement=NULL;
			for(i=0;i<len;i++)
			{
				filtredElement=ll_get(this,i);
				if(fn(filtredElement))
				{
					ll_add(FiltredList,filtredElement);
				}
			}
		}
	}
